...
<style>
    ...
    .checkmark { width: 80px; height: 80px; border-radius: 50%; display: block; stroke-width: 2; stroke: #0ECB81; stroke-miterlimit: 10; animation: fill .4s ease-in-out .4s forwards; }
    .checkmark__circle { stroke-dasharray: 166; stroke-dashoffset: 166; stroke-width: 2; stroke-miterlimit: 10; stroke: #0ECB81; fill: none; animation: stroke 0.6s forwards; }
...
def live_ui():
    ...
    h2.markdown(f'<div class="data-card"><div class="card-label">{coin} 实时现价</div><div class="card-value">${(curr_p if curr_p else 0):,.2f}</div></div>', unsafe_allow_html=True)

    # 3. K 线区域 (包含 MACD 增加)
    if chart_mode == "TradingView":
        tv_i = "1" if k_interval == "1m" else k_interval.replace("m", "")
        tv_html = f"""<div style="height:450px;"><div id="tv" style="height:450px;"></div><script src="https://s3.tradingview.com/tv.js"></script><script>new TradingView.widget({{"autosize":true,"symbol":"BINANCE:{coin}","interval":"{tv_i}","theme":"light","style":"1","locale":"zh_CN","container_id":"tv","studies":["BB@tv-basicstudies","MACD@tv-basicstudies"]}});</script></div>"""
        components.html(tv_html, height=450)
    else:
        df_k, _ = get_klines_smart_source(coin, k_interval)
        if not df_k.empty:
            # --- 核心修改：指标计算 ---
            # BOLL
            ...
            # 1. 布林带 (深蓝与粉色加粗)
            fig.add_trace(go.Scatter(x=df_k['time'], y=df_k['up'], line=dict(color='#1f77b4', width=2.5), name='BOLL上(深蓝)'), row=1, col=1)
            fig.add_trace(go.Scatter(x=df_k['time'], y=df_k['dn'], line=dict(color='#e377c2', width=2.5), name='BOLL下(粉色)'), row=1, col=1)
            fig.add_trace(go.Candlestick(x=df_k['time'], open=df_k['open'], high=df_k['high'], low=df_k['low'], close=df_k['close'], increasing_fillcolor='#0ECB81', decreasing_fillcolor='#F6465D', name='K线'), row=1, col=1)
            
            # 2. MACD 副图
            fig.add_trace(go.Bar(x=df_k['time'], y=df_k['hist'], marker_color='gray', name='Histogram'), row=2, col=1)
            fig.add_trace(go.Scatter(x=df_k['time'], y=df_k['macd'], line=dict(color='#2962FF', width=1), name='MACD'), row=2, col=1)
            fig.add_trace(go.Scatter(x=df_k['time'], y=df_k['signal'], line=dict(color='#FF6D00', width=1), name='Signal'), row=2, col=1)

            fig.update_layout(
                height=480, margin=dict(t=5,b=5,l=0,r=0), 
                xaxis_rangeslider_visible=False, 
                dragmode='pan', # 开启单指滑动查看历史
                plot_bgcolor='white', paper_bgcolor='white',
                xaxis=dict(fixedrange=False), # 开启双指/滚轮缩放
    ...
    if st.session_state.orders:
        ...
        for o in reversed(st.session_state.orders[-10:]):
            ...
            t_d.append({
                "币种": o['资产'].replace("USDT",""), "方向": o['方向'], "金额": f"${o['金额']}", 
                "开仓": f"{o['开仓价']:,.2f}", "结果/倒计时": o['结果'] if o['结果'] else f"{int(max(0,rem))}s"
            })
        st.table(t_d)
