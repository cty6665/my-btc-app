...
.card { background:#F8F9FA; border-radius:8px; padding:10px; border:1px solid #EEE; }
...
fig.add_scatter(x=df["time"], y=df["BOLL_UP"], line=dict(color="#FCD535", width=1), row=1, col=1)
fig.add_scatter(x=df["time"], y=df["BOLL_DN"], line=dict(color="#FCD535", width=1), row=1, col=1)
fig.add_scatter(x=df["time"], y=df["MA20"], line=dict(color="#888", width=1), row=1, col=1)

# MACD
fig.add_bar(x=df["time"], y=df["MACD"], marker_color="#999", row=2, col=1)
...
if col1.button("🟢 看涨 UP"):
    if st.session_state.balance >= bet:
        ...
        st.session_state.orders.append({
            "side":"UP","entry":price,"amount":bet,
...
if col2.button("🔴 看跌 DOWN"):
    if st.session_state.balance >= bet:
        st.session_state.balance -= bet
        st.session_state.orders.append({
            "side":"DOWN","entry":price,"amount":bet,
            "settle":now+timedelta(minutes=duration),
            "status":"OPEN","result":None
        })
        st.rerun()
