...
with st.sidebar:
    st.header("⚙️ 设置")
    chart_mode = st.radio("K线引擎", ["TradingView (VPN)", "原生绘制 (直连)"], key="chart_engine")
    coin = st.selectbox("币种", ["BTCUSDT", "ETHUSDT", "SOLUSDT"], key="coin_select")
    duration = st.radio("周期", [5, 10, 30], format_func=lambda x:f"{x}min", key="dur_select")
    bet = st.number_input("下单金额", 10.0, 1000.0, 50.0, key="bet_val")
    if st.button("🚨 重置记录", key="reset_btn"):
...
with chart_placeholder.container():
    if chart_mode == "TradingView (VPN)":
        ...
        if status == "OK":
            ...
            fig.add_trace(go.Scatter(x=df_k['time'], y=df_k['upper'], line=dict(color='rgba(0,0,255,0.1)'), name='BB'), row=1, col=1)
            fig.add_trace(go.Scatter(x=df_k['time'], y=df_k['lower'], line=dict(color='rgba(0,0,255,0.1)'), fill='tonexty'), row=1, col=1)
...
if col_up.button("🟢 看涨 (UP)", key="up_btn") and current_price:
    if st.session_state.balance >= bet:
        ...
        st.toast(f"✅ 看涨开仓成功！现价: {current_price}", icon="📈")
        st.rerun()

if col_down.button("🔴 看跌 (DOWN)", key="down_btn") and current_price:
    if st.session_state.balance >= bet:
        st.session_state.balance -= bet
        ...
        save_db(st.session_state.balance, st.session_state.orders)
        st.toast(f"✅ 看跌开仓成功！现价: {current_price}", icon="📉")
        st.rerun()
