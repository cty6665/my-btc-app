...
with st.sidebar:
    ...
    duration = st.radio("结算周期", [5, 10, 30], format_func=lambda x: f"{x} 分钟")
...
def live_ui():
    ...
    h2.markdown(f'<div class="data-card"><div class="card-label">{coin} 现价</div><div class="card-value">${(curr_p if curr_p else 0):,.2f}</div></div>', unsafe_allow_html=True)

    # 3. K 线展示
    if chart_mode == "TradingView":
        tv_i = "1" if k_interval == "1m" else k_interval.replace("m", "")
        tv_html = f"""<div style="height:420px;"><div id="tv" style="height:420px;"></div><script src="https://s3.tradingview.com/tv.js"></script><script>new TradingView.widget({{"autosize":true,"symbol":"BINANCE:{coin}","interval":"{tv_i}","theme":"light","style":"1","locale":"zh_CN","container_id":"tv","studies":["BB@tv-basicstudies"]}});</script></div>"""
        ...
        if not df_k.empty:
            ...
            # 主图：布林带
            fig.add_trace(go.Scatter(x=df_k['time'], y=df_k['up'], line=dict(color='#1f77b4', width=2), name='上轨(深蓝)'), row=1, col=1)
            ...
            fig.update_layout(
                height=450, margin=dict(t=5,b=5,l=0,r=0), 
    ...
    if st.session_state.orders:
        ...
        for o in reversed(st.session_state.orders[-10:]):
            ...
            t_d.append({
                ...
                "开仓": f"{o['开仓价']:,.2f}", "结果/倒数": o['结果'] if o['结果'] else f"{int(max(0,rem))}s"
            })
        st.table(t_d)
