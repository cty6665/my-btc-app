...
.metric-card {{
    background: {theme['card']}; padding: 15px; border-radius: 8px;
...
def commit_order_callback(direction: str):
    ...
    st.session_state.orders.append({
        "资产": st.session_state.coin, "方向": direction, "开仓价": c_price,
        "金额": st.session_state.bet_amt, "开仓时间": open_time,
        "结算time": settle_time, "状态": "待结算"
    })
...
def operations_fragment():
    ...
    c1.markdown(
        f'<div class="metric-card"><div class="metric-label">账户净资产 (USDT)</div>'
        f'<div class="metric-value" style="color:{theme["brand"]};">'
        f'${st.session_state.balance:,.2f}</div></div>',
        unsafe_allow_html=True
    )
    c2.markdown(
        f'<div class="metric-card"><div class="metric-label">WS 实时标记价格</div>'
        f'<div class="metric-value">${live_price:,.4f}</div></div>',
        unsafe_allow_html=True
    )
    c3.markdown(
        f'<div class="metric-card"><div class="metric-label">当前运行币种</div>'
...
with col_2:
    st.session_state.interval = st.selectbox("跨度", ["1m", "3m", "5m", "15m", "30m", "1h"])
with col_3:
...
with col_4:
    st.session_state.bet_amt = st.number_input(
        "委托金额 (U)", min_value=10.0, step=50.0, value=st.session_state.bet_amt
    )
...
if not closed:
    ...
    for o in sorted(closed, key=lambda x: x["结算time"]):
        ...
        equity_data.append({"time": int(o["结算time"].timestamp()), "value": round(running, 2)})
